...
import { Col, Row, Button, List, Form, message, Input, Typography, DatePicker, Modal } from "antd";
...
export default function Main() {
  ...
  const getRecomendation = () => {
    ...
    axios.get(`https://62e18fede8ad6b66d84c2b6f.mockapi.io/recomendations`)
      .then(function (response) {
  ...
  const onFinishRecomendations = (values) => {
    if (values.recomendationName === undefined) {
      ...
      axios.post(`https://62e18fede8ad6b66d84c2b6f.mockapi.io/recomendations`, {
  ...
  const onDelete = (item) => {
    axios.delete(`${API_URL}/movies/${item.id}`)
      .then(function (response) {
        getMovies();
        localStorage.setItem('lastUpdated', currentTime);
      })
  ...
  const onDeleteSeries = (item) => {
    axios.delete(`${API_URL}/series/${item.id}`)
      .then(function (response) {
        getSeries();
        localStorage.setItem('lastUpdated', currentTime);
      })
  ...
  const onDeleteRecomendation = (item) => {
    axios.delete(`https://62e18fede8ad6b66d84c2b6f.mockapi.io/recomendations/${item.id}`)
      .then(function (response) {
        getRecomendation();
        localStorage.setItem('lastUpdated', currentTime);
      })
  ...
  return (
    <div className="container-all">
      <main className="main-container">
        <div className="movies">
          ...
          >
            <div className="inputs">
              ...
              <Form.Item name="movieDate">
                <DatePicker onChange={onChange} format="DD/MM/YYYY" placeholder="Selecione a data"/>
              </Form.Item>
         ...
         <Col className="list-container-col">
            <List
              ...
              loading={loading}
              locale={{ emptyText: <div className="no-data"><h1>Sem filmes registrados</h1></div> }}
              renderItem={(item, index) => (
                <List.Item>
                  <Col span={24}>
                    <Row className="list-row">
                      <Col span={14}>
                        <p style={{ display: 'inline', fontSize: '18px' }}>{item.name}</p>
                      </Col>
        ...
        <div className="series">
          ...
          <Col className="list-container-col">
            <List
              itemLayout="horizontal"
              dataSource={dataSeries.at(-1)}
              loading={loadingSeries}
              locale={{ emptyText: <div className="no-data"><h1>Sem series registradas</h1></div> }}
              renderItem={(item, index) => (
                <List.Item>
                  <Col span={24}>
                    <Row className="list-row">
                      ...
                      <Col span={3}>
                        <Button icon={<MinusOutlined />} onClick={() => onDeleteSeries(item)} />
                      </Col>
                    </Row>
                  </Col>
                </List.Item>
      ...
      <footer className="footer-container">
        ...
        <Modal title="Recomendações" visible={isModalVisible} onOk={handleOk} onCancel={handleCancel}>
          <Form
          ...
          <Col className="list-container-col">
            <List
              ...
              locale={{ emptyText: <div className="no-data"><h1>Sem recomendações registradas</h1></div> }}
              renderItem={(item, index) => (
                <List.Item>
                  <Col span={24}>
                    <Row className="list-row">
                      ...
                      <Col span={3}>
                        <Button icon={<MinusOutlined />} onClick={() => onDeleteRecomendation(item)} />
                      </Col>
                    </Row>
                  </Col>
                </List.Item>
